...
int meshToVoxels() {
	...
	// tree leaves
	octree.setLeavesSize(0x80000000u / sizeof(SVO::Leaf));
	GLuint leafBuffer;
	glGenBuffers(1, &leafBuffer);
	glBindBuffer(GL_SHADER_STORAGE_BUFFER, leafBuffer);
	glBufferData(GL_SHADER_STORAGE_BUFFER, 0x80000000u, octree.getLeafData(), GL_DYNAMIC_COPY);
